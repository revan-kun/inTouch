package com.epam.lab.intouch.web.util;

public class Attribute {
	public static final String MEMBER_NAME = "memberName";
	public static final String MEMBER_SURNAME = "memberSurname";
	public static final String MEMBER_LOGIN = "memberLogin";
	public static final String MEMBER_PASSWORD = "memberPassword";
	public static final String MEMBER_BIRTHDAY = "memberBirthday";
	public static final String MEMBER_REGISTRATION_DATE = "memberRegistrationDate";
	public static final String MEMBER_SEX = "memberSex";
	public static final String MEMBER_QUALIFICATION = "memberQualification";
	public static final String MEMBER_EXPERIENCE = "memberExperience";
	public static final String MEMBER_PHOTO = "memberPhoto";
	public static final String MEMBER_PROJECT_ROLE = "memberProjectRole";

	public static final String PROJECT_ID = "projectID";
	public static final String PROJECT_NAME = "projectName";
	public static final String PROJECT_CREATED = "projectCreationDate";
	public static final String PROJECT_ESTIMATED_COMPLETION = "projectEstimatedCompletion";
	public static final String PROJECT_COMPLETED = "projectCompleted";
	public static final String PROJECT_DESCRIPTION = "projectDescription";
	public static final String PROJECT_CUSTOMER = "projectCustomer";
	public static final String PROJECT_STATUS = "projectStatus";
}
